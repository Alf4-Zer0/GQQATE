function [ModelDDSIMCA, TestDDSIMCA, LabelModel, LabelTest] = DS_DDSIMCA(varargin)
    % DS_DDSIMCA aplica Kennard-Stone apenas no primeiro conjunto de dados (S1), enquanto todos os outros são dados de teste.
    % -------------------------------------------------------------------------
    % Uso:
    %   [ModelDDSIMCA, TestDDSIMCA, LabelModel, LabelTest] = DS_DDSIMCA(S1, LabelS1, S2, LabelS2, ...)
    % -------------------------------------------------------------------------
    % Entradas:
    ...
    % -------------------------------------------------------------------------
    % Saídas:
    %   - ModelDDSIMCA : Conjunto de dados contendo dados de treino (de S1)
    %   - TestDDSIMCA  : Conjunto de dados contendo dados de teste (de S1 e todos os outros conjuntos de dados)
    %   - LabelModel : Rótulos para ModelDDSIMCA
    %   - LabelTest  : Rótulos para TestDDSIMCA
    % -------------------------------------------------------------------------
    % Exemplo:
    %   [ModelDDSIMCA, TestDDSIMCA, LabelModel, LabelTest] = DS_DDSIMCA(data1, labels1, data2, labels2, data3, labels3);
    % -------------------------------------------------------------------------
    % Escrito por:
    ...
    for i = 2:numDatasets
        ...
        TestDDSIMCA = cat(1, double(TestDDSIMCA), X); % Converter para double para evitar erros
